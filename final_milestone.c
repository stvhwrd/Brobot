...
//////// SPEEDS ////////
#define PRIMARY_SCAN_TURN_SPEED     30    // speed of primary scan turn (motor value 0 - 127)
#define SECONDARY_SCAN_TURN_SPEED   25    // speed of secondary scan turn (motor value 0 - 127)
#define TURNAROUND_SPEED            30    // speed of 90 degree turn (motor value 0 - 127)
#define REVERSE_SPEED               25    // absolute speed of reverse drive - should not be negative (motor value 0 - 127)
#define FORWARD_SPEED               40    // speed of forward drive in open space (motor value 0 - 127)
#define SLOW_SPEED                  20    // speed at which to creep forward when in close range (motor value 0 - 127)

//////// TIMING /////////
#define PRIMARY_SCAN_TIME           4000  // time-length of first high speed scan (milliseconds)
#define SECONDARY_SCAN_TIME         6000  // time-length of the second lower speed scan (milliseconds)
#define TURNAROUND_TIME             1000  // calibrated value - 90 degree turn (milliseconds)
#define REVERSE_TIME                1500  // time-length of reverse drive (milliseconds)
...
int IRMax     = 0;    // highest value of IR observed.  We assume this is coming from the beacon.
...
{
    stopWheels();
    wait1Msec(400);   // brief pause to prevent rocking
...
int medianFilter()
{
    ...
    // (using short circuit exhaustive comparisons to find median)
...
/**
*   standingBy():   Default machine state.  Robot stands by while polling for
*                   a button push.
*   input:          none
...
/*
*   primaryScan():  Seek out beacon by rotating and sampling for IR, then turn
*                   to focus on point of maximum IR emission.
*   input:          none
...
State secondaryScan()
{
    IRMax = 0;  // highest value that the IR sensor is exposed to
...
task main()
{
    ...
    while (true)
    {
        // all switch statements are pretty self-explanatory COMMAND --> action 
        // i.e. when state is STANDBY, function is "standingBy"
        switch (machineState)
        {
        case (STANDBY):
